...
doubles []     = 0
doubles [x]    = 0
doubles (x:xs) = doubles' x x 0 xs

-- Helper function for doubles
doubles' :: Int -> Int -> Int -> [Int] -> Int
doubles' first last acc [] 
    | first == last = acc + first
    | otherwise     = acc
doubles' first last acc (x:xs)
    | x == last = doubles' first x (acc + x) xs
    | otherwise = doubles' first x acc xs