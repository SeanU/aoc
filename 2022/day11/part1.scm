...
(define (take-turn monkeys active-monkey)
  (let [(items (monkey-items active-monkey))
    ...
    (define (handle-item item)
      (let* [(worry-level (quotient (operation item) 3))
             (test-result (= 0 (modulo worry-level test-number)))
             (next-monkey (if test-result
                              true-monkey-items
                              false-monkey-items))]
        (list-queue-add-back! next-monkey worry-level)
...
(define (do-round monkeys)
  (for-each (lambda (m) (take-turn monkeys m)) monkeys))
...
(let* [(input (call-with-input-file "./2022/day11/input.txt" read-lines))
  ...
  (range-for-each (lambda (_) (do-round monkeys))
                  (numeric-range 0 20))
  (newline)
