...
(define (apply-instruction state instruction-text)
  (let* [(parts (string-split instruction-text))
         ...
         (num (string->number (second parts)))
         (crane '())]
    (begin
      (do [(i num (- i 1))]
          ((= i 0))
        (set! crane (cons (car (vector-ref state from)) crane))
        (vector-set! state
                     from
      ...
      (do [(i num (- i 1))]
        ...
        (vector-set! state
                     to
                     (cons (car crane) (vector-ref state to)))
        (set! crane (cdr crane))))))
...
(let* [(lines (call-with-input-file "./2022/day05/input.txt" read-lines))
