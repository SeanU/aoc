...
int longest_route_from(struct CityList cities, struct Map map, char *here)
{
    ...
    for (int i = 0; i < cities.count; i++)
    {
        ...
        if (there != NULL)
        {
            ...
            rest.cities[i] = NULL;
            dist += longest_route_from(rest, map, there);
            // printf("%d\n", dist);
            if (dist > longest)
            {
...
int longest_route(struct CityList cities, struct Map map)
{
    int longest = 0;
    ...
    for (int i = 0; i < cities.count; i++)
    {
        ...
        struct CityList rest = clone(cities);
        rest.cities[i] = NULL;
        int dist = longest_route_from(rest, map, here);
        if (dist > longest)
        {
            longest = dist;
        }
...
int main(void)
{
    ...
    // }
    int longest = longest_route(cities, map);
    printf("Longest route length: %d\n", longest);
