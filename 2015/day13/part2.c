...
int main(int argc, char **argv)
{
    ...
    size_t new_n_rules = n_rules + (guests.number * 2);
    rules = realloc(rules, new_n_rules * sizeof(struct Rule));
    for (size_t i = 0; i < guests.number; i++)
    {
        char *guest = guests.names[i];
        rules[n_rules] = (struct Rule){.person = "Me", .neighbor = guest, .d_happy = 0};
        n_rules += 1;
        rules[n_rules] = (struct Rule){.person = guest, .neighbor = "Me", .d_happy = 0};
        n_rules += 1;
    }
    guests = add_guest(guests, "Me");
